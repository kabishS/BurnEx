...
async function renderProfile(){
  ...
  document.getElementById('profWorkouts').textContent = isAdmin ? '—' : (await allSessions(u.username)).length;

  // Gym owners don't track a body or workouts of their own — hide BMI and
  // the weight/height/age/gender/goal fields, which only feed BMI/BMR/
  // calorie math for members.
  document.getElementById('profBmiWrap').classList.toggle('d-none', isAdmin);
  document.getElementById('memberProfileFields').classList.toggle('d-none', isAdmin);
  document.querySelectorAll('#memberProfileFields input, #memberProfileFields select').forEach(el => el.required = !isAdmin);
