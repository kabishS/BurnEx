...
function switchRole(role){
  ...
  // Weight/height/age/gender only matter for a member's BMI/BMR/calorie
  // math — a gym owner account has no body-tracking of its own.
  document.getElementById('memberFieldsGroup').classList.toggle('d-none', role!=='member');
  ...
  const required = role === 'member';
  ageInput.required = required;
...
async function handleRegister(e){
  ...
  try {
    ...
    let gymCode = null;
    let age = null, gender = null, weight = null, height = null;
    ...
    await DB.createUser({
      ...
      age, gender, weight, height, goal: role === 'member' ? 400 : null,
